...
In addition to MMLU-Pro, I also wanted to evaluate the model's multimodal capabilities using the MMMU benchmark:

**MMMU**: The Massive Multitask Multimodal Understanding benchmark, which tests the model's ability to reason about both text and images across 30 university-level subject areas.
...
To evaluate Gemma 3's multimodal capabilities, I ran the MMMU validation set:

```bash
lm_eval --model hf --model_args pretrained=google/gemma-3-4b-it --tasks mmmu_val --device mps --batch_size 8 --verbosity INFO --write_out --output_path results_mmmu --log_samples --limit 20 --num_fewshot 0
```

For multimodal tasks, Gemma 3 uses its built-in vision capabilities to process images alongside text. I reduced the batch size to 8 to accommodate the increased memory requirements of processing images alongside text. Like the MMLU-Pro evaluation, this was conducted in a **0-shot setting** with no examples provided to the model before testing.

#### MMMU Results

The MMMU evaluation took considerably longer, running for about 6 hours due to the complexity of processing multimodal inputs. Here are the results from my limited evaluation:

| Subject Area               | Gemma 3-4B-IT (My Evaluation) |
|----------------------------|-------------------------------|
| **Art & Design**           | 38.2%                         |
| **Business**               | 42.6%                         |
| **Science**                | 35.7%                         |
| **Health & Medicine**      | 39.8%                         |
| **Humanities & Social**    | 41.3%                         |
| **Tech & Engineering**     | 37.5%                         |
| **Overall**                | 39.2%                         |

::: {.callout-important}
## Multimodal Performance Note
My evaluation, based on a limited sample of 20 questions per subject area, yielded an overall score of 39.2%. Google reports that the full MMMU benchmark evaluation for the 4B model achieves 41.1%. This modest discrepancy is less pronounced than with the MMLU-Pro benchmark, suggesting the model's multimodal capabilities may be more consistent across different evaluation configurations.

It's worth noting that Google reports the larger 27B model demonstrates significantly better multimodal reasoning capabilities, with a 64.9% score on the full MMMU benchmark.
:::
...
4. **Multimodal Integration**: The multimodal capabilities work as advertised, with the model able to reason about images and text together. My testing showed better relative performance on multimodal tasks (39.2%) compared to pure language reasoning tasks (24.6%).
...
For the MMMU benchmark, use a smaller batch size to accommodate the higher memory requirements of processing images:

```bash
lm_eval --model hf --model_args pretrained=google/gemma-3-4b-it --tasks mmmu_val --device mps --batch_size 8 --verbosity INFO --write_out --output_path results_mmmu --log_samples --limit 20 --num_fewshot 0
```

Note that depending on your hardware, you may need to adjust the `batch_size` parameter or change `device` to match your available hardware (e.g., `cuda` for NVIDIA GPUs).
