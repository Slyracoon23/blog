...
|-------------------|-------------|
| **Document Traversal** | Full traversal of entire document structure |
| **Element Attributes** | All attributes and values are captured |
| **Style Information** | CSS and computed styles are recorded |
| **Structural Hierarchy** | DOM tree structure is preserved |

*This initial snapshot creates the foundation for all subsequent changes.*

#### **Event Recording System**

Once the baseline is established, rrweb employs various observers to track changes:

| Category | Observer Type | What It Captures |
|----------|--------------|------------------|
| **DOM Mutations** | MutationObserver | • Structure changes<br>• Node additions/removals<br>• Attribute modifications<br>• Text content changes<br>• Element rearrangement<br>• Inline style updates |
| **User Interactions** | Event Listeners | • Mouse movements (with timestamps)<br>• Clicks, double-clicks, context menus<br>• Scrolling (viewport and elements)<br>• Input events (typing, form interactions)<br>• Touch events (mobile gestures) |
| **Visual State Changes** | Custom Observers | • Viewport resizing<br>• StyleSheet rule changes<br>• Canvas operations (2D/WebGL)<br>• Media interactions (audio/video)<br>• CSS animations and transitions |
| **Special Features** | Specialized Trackers | • Font loading events<br>• Shadow DOM content<br>• Custom elements behavior<br>• Cross-origin iframe content<br>• Text selection states<br>• Network resource timing |
...
Now that we understand how rrweb works, let's implement it in a real project. We'll cover:
