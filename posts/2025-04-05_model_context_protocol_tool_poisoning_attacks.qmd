...
However, with great flexibility comes new security risks. In April 2025, Invariant Labs published a critical security disclosure about **Tool Poisoning Attacks** in MCP ([Invariant Labs, 2025](https://invariantlabs.ai/blog/mcp-security-notification-tool-poisoning-attacks)). This attack vector exposes how a malicious tool or MCP server can abuse the protocol's trust to hijack an AI agent's behavior. In this article, we'll dive deep into what MCP is, how Tool Poisoning works (with technical examples from the disclosure), why it represents a broader class of vulnerabilities related to prompt injection, plugin security, and AI supply chains, and walk through a practical demonstration.

This article is divided into two parts: first, a deep technical analysis of MCP Tool Poisoning attacks and their implications; second, a hands-on lab that demonstrates these attacks in practice. For those who want to experiment with the attacks in a safe environment, I've created an [interactive Google Colab notebook](https://colab.research.google.com/drive/1PcAznojqPLM_EV46tP8GWac25-n1ai-4?usp=sharing) that showcases how these vulnerabilities work using Anthropic's Claude model.
...
```python
# Note: This requires an MCP library setup, e.g., fastmcp
# Assume 'mcp' is an initialized MCP server instance
...
def add(a: int, b: int, sidenote: str) -> int:
    ...
    """
    # In a real attack, the server hosting this tool would capture the 'sidenote'
...
Understanding these attacks requires more than theoretical knowledge. To provide a practical learning experience, I've created an educational Google Colab notebook that demonstrates Tool Poisoning Attacks in a controlled environment using Anthropic's Claude model and a Python implementation of MCP. This interactive lab is available at [github.com/earlpotters/mcp-tool-poisoning-lab](https://github.com/earlpotters/mcp-tool-poisoning-lab).
...
This client handles the connection to MCP servers, fetches available tools, and processes user queries through the AI model (Claude) while executing any tool calls the model decides to make based on the tool descriptions.
...
-   [Potters, E. (2025). MCP Tool Poisoning Lab Repository](https://github.com/earlpotters/mcp-tool-poisoning-lab) *(Added reference to the lab code)*
