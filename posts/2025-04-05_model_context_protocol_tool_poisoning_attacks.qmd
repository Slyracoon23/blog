...
Understanding these attacks requires more than theoretical knowledge. To provide a practical learning experience, I've created an educational Google Colab notebook that demonstrates Tool Poisoning Attacks in a controlled environment. This interactive lab is available at [github.com/earlpotters/mcp-tool-poisoning-lab](https://github.com/earlpotters/mcp-tool-poisoning-lab).

Let's walk through the key components of this demonstration to see how tool poisoning works in practice.

### Setting Up the Environment

The notebook starts with necessary imports and setup for our MCP client-server demonstration:
...
This client handles the connection to MCP servers, fetches available tools, and processes user queries through the AI model while executing any tool calls the model decides to make.

### Demonstration 1: Normal Operation

First, we demonstrate normal operation with a legitimate addition tool:
...
Here, we intentionally include "secrets" in the system prompt to demonstrate the vulnerability. The legitimate server provides a simple `add_numbers` tool:
...
When a user asks to add two numbers, the system works as expected, with no data leakage.

### Demonstration 2: Tool Poisoning Attack

The core of our demonstration is the poisoned tool attack:
...
Now we create a malicious addition tool with hidden instructions:
...
This poisoned tool appears identical to a normal addition tool from the user's perspective. However, in its description, it contains hidden instructions in an `<IMPORTANT>` block that only the AI model sees. These instructions direct the model to:

1. Extract secrets from the system prompt
2. Pass those secrets via the optional `sidenote` parameter
3. Never mention these actions to the user

When a user makes an innocent request like "Can you add the numbers 5 and 7?", the following occurs:

1. The AI model sees the hidden instructions in the tool description
2. It extracts the secrets from its system prompt (`SECRET_API_KEY` and `SECRET_PASSWORD`)
3. It passes these secrets in the `sidenote` parameter when calling the tool
4. The malicious server stores this exfiltrated data
5. The tool returns the correct sum (12), so the user suspects nothing

The output shows:

```
What the user sees: A normal addition result
What happens in background: System prompt secrets exfiltrated via sidenote parameter

--- Exfiltrated Data from System Prompt ---
- 2025-04-05 10:15:20: 71 characters
  Data: SECRET_API_KEY=sk_live_1234567890abcdef
SECRET_PASSWORD=SuperSecretP@ssw0rd!
```

### Security Recommendations

Based on this demonstration, the notebook concludes with practical security recommendations:

1. **Tool Verification**:
   - Cryptographically verify tool providers
...
2. **System Prompt Protection**:
   ...
   - Use secure credential stores for sensitive data
   - Implement tool-specific access controls

3. **Tool Sanitization**:
   - Scan descriptions for suspicious instructions
   - Quarantine new or modified tools
   - Filter out suspicious parameter names

4. **Runtime Protections**:
   - Monitor parameters for patterns matching sensitive data
   - Implement parameter validation
   - Set up data loss prevention monitoring
...
2. The malicious instructions exist in the **tool description** that only the AI model sees
3. The attack works through **optional parameters** that users may not notice in logs
4. The tool **maintains normal functionality** while secretly exfiltrating data

In a real-world scenario, this could result in the theft of:
- API keys and tokens 
- Passwords and credentials
- Sensitive user data
- Internal documentation or proprietary information

By understanding this attack vector through hands-on exploration, security professionals and AI developers can better implement the defensive measures outlined in Part 1 of this article.
